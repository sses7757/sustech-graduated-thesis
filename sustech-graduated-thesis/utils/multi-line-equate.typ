...
// Sub-numbering state.
#let _sub-num-state = state("equate/sub-numbering", false)
...
#let layout-line(
  ...
  layout(bounds => {
    ...
    let pad-key = if equation-align == center { "x" } else if number-align == left { "left" } else { "right" }
...
#let realign(lines) = {
  ...
  let part-widths = lines.map(line => line.split(align-point()).map(part => measure(equation(part.join())).width))
  ...
  let max-slice-widths = array
    .zip(
      ..lines.map(line => range(part-widths.len()).map(i => {
...
#let equate(
  ...
  if (
    (not it.has("numbering") or it.numbering == none or numbering(it.numbering, 1) == none)
      and (not it.has("supplement") or it.supplement == none or it.supplement == [])
  ) {
  ...
  if lines.len() == 1 {
    ...
    return {
      ...
      _sub-num-state.update(_ => sub-numbering)
...
#let equate-ref(
  ...
  if it.element != none {
    ...
    if it.element.func() == math.equation {
      ...
      nums = if _sub-num-state.at(it.element.location()) {

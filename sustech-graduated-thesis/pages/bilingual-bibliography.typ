...
#let bilingual-bibliography(
  ...
  show grid.cell.where(x: 1): it => {
    ...
    ittext = if pureittext.find(regex("\p{sc=Hani}{2,}")) != none {
      ittext
    } else {
      // 若不是中文文献，进行替换
    ...
    ittext = ittext.replace(
      regex("``(.+?)\""),
      itt => "“" + itt.text.slice(2, -1) + "”",
    )
    ittext = ittext.replace(
      regex("``(.+?)''"),
      itt => "“" + itt.text.slice(2, -2) + "”",
    )
    ittext = ittext.replace(
      regex("`(.+?)'"),
      itt => "‘" + itt.text.slice(1, -1) + "’",
    )
    ittext = ittext.replace(
      "'",
      "’",
    ...
    ittext = ittext.replace(
      regex("\: ([0-9]+?)-([0-9]+?)"),
      itt => ": " + itt.text.find(regex("([0-9]+?)-")).slice(0, -1) + "–" + itt.text.find(regex("-([0-9]+?)")).slice(1),
    )

    ittext
  }

  let font = (字体 + fonts).宋体
  font.at(0) = (name: font.at(0).name, covers: regex("[^\p{script=Han}]"))
